...
public class ClearDesktopBackupTask {
	...
	public static String clearDesktopAndBackup(String query, int firstFlag) throws Exception {
		...
		if(!backupPath.isEmpty() && firstFlag != 1) {
		...
		if (!backupPath.isEmpty() && (backupPath.contains("\\") || backupPath.contains("/"))) {
		...
		try {
			List<Path> filesAndFolders = Files.list(backupPathFolder).filter(path -> !path.equals(backupPathFolder)) // Exclude
																											// the
																											// desktop
																											// folder
																											// itself
			...
			// folder)
			removeFilesAndFolders(backupPathFolder, backupFolder); // Exclude the backup folder and files inside it

			// System.out.println("Desktop has been cleared, excluding .exe, .lnk files, and
			// the backup folder."); refreshing desktop
			if (desktopFlag) {
	...
	// backup folder)
	private static void removeFilesAndFolders(Path backupPathFolder, Path backupFolder) throws IOException {
		try (Stream<Path> paths = Files.walk(backupPathFolder)) {
			paths.filter(Files::exists).filter(path -> !path.equals(backupPathFolder)) // Exclude the desktop folder itself
					.filter(path -> !path.equals(backupFolder)) // Exclude the backup folder
